...
char commandsNames[][MAX_ARG_LEN]={"test_sync","test","nice","unblockPid","blockPid","ps","getPid","kill","time","help","inforeg","chess","printmem","divZero","invalidOPCode","clear","echo","UWU"};
void  (* run[])(char args[MAX_ARGS][MAX_ARG_LEN]) = {semTester,test,nice,unblockPid,blockPid,ps,getPid,kill,time,help,inforeg,chessO,printmem,divZero,invalidOPCode,clear,echo,uwu};
